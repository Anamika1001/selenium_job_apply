...
def abort_application():
    ...
    discard_button = driver.find_elements(
        by=By.CLASS_NAME, value="artdeco-modal__confirm-dialog-btn")[1]
...
chrome_driver_path = ChromeDriverManager(
    path="YOUR CHROME DRIVER FOLDER").install()
...
for listing in all_listings:
    ...
    try:
        ...
        phone = driver.find_element(
            by=By.CSS_SELECTOR, value="input[id*=phoneNumber]")
        if phone.text == "":
            phone.send_keys(PHONE)

        # Check the Submit Button
        submit_button = driver.find_element(
            by=By.CSS_SELECTOR, value="footer button")
